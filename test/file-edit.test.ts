...
test("PUT /api/agents/:id/file — write, create, conflict, binary, escape", async () => {
  await useTempDirs(["fileedit-root-", "fileedit-ws-"], async ([dataDir, ws]) => {
...
test("GET /raw serves media bytes with correct mime; rejects non-media", async () => {
  await useTempDirs(["raw-root-", "ws-"], async ([dataDir, ws]) => {
...
test("tree marks gitignored entries; non-git workspaces don't", async () => {
  await useTempDirs(["gi-root-", "ws-"], async ([dataDir, ws]) => {
    const { writeFile } = await import("node:fs/promises");
